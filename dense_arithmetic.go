...
func (m DenseMatrix) Log() *DenseMatrix {
	out := Zeros(m.Rows(), m.Cols())

	for ri := 0; ri < m.Rows(); ri++ {
		for ci := 0; ci < m.Cols(); ci++ {
			out.Set(ri, ci, math.Log2(m.Get(ri, ci)))
		}
	}
	return out
}

func (m DenseMatrix) Pow(power float64) *DenseMatrix {
	out := Zeros(m.Rows(), m.Cols())

	for ri := 0; ri < m.Rows(); ri++ {
		for ci := 0; ci < m.Cols(); ci++ {
			out.Set(ri, ci, math.Pow(m.Get(ri, ci), power))
		}
	}
	return out
}
